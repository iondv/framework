...
## Выделение цветом объектов в коллекции

Функционал применяется в случаях, когда необходимо выделить цветом объекты в таблице атрибута типа "Коллекция", в соответствии с каким либо признаком. Условия для выделения цветом объектов задаются аналогично [условиям для выделения цветом объектов в списках](/docs/ru/2_system_description/metadata_structure/meta_view/styles.md).

Настройка задается в мете представления класса для атрибута типа "Коллекция". Cвойство `"options"`.  

### Пример

```
{
  "caption": "Поле с коллекцией",
  "type": 3,
  "property": "testCol",
  ...
  "columns": [
