...
функциональность, визуальной среды визуальной среды [Studio](https://studio.iondv.com) для разработки метаданных приложений [с открытым кодом](https://github.com/iondv/studio). Смоделированное в виде UML-схемы веб-приложение можно запустить за [80 секнуд](https://youtu.be/s7q9_YXkeEo).
...
Фреймворк подходит для создания систем с накоплением и обработкой больших данных и искусственного интеллекта, для этого разрабатывается специальный модуль [Artificial Intelligence Bus](https://github.com/iondv/aib) и демонстрационные преложения, например разметки данных и обучения [моделей TensorFlow](https://github.com/iondv/tensorflow-dataset).
