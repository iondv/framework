...
module.exports = (env) => {
  ...
  app.use((req, res, next) => {
    ...
    for (let i = 0; i < locales.length; i++) {
      if (app.locals.supportedLanguages.includes(locales[i].code)) {
        lang = locales[i].code;
        break;
      } else if (app.locals.supportedLanguages.includes(locales[i].language)) {
        lang = locales[i].language;
        break;
      }
    }
    req.locals = {};
    res.locals.lang = req.locals.lang = lang;
    res.locals.__ = req.locals.__ = (str, params) => strings.s('tpl', str, params, lang);
    res.locals.t = req.locals.t = (str, ...args) => t(str, ...args, {lang: req.locals.lang});
    next();
  });
