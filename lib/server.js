...
module.exports = (env) => {
  ...
  app.use((req, res, next) => {
    ...
    req.locals.lang = res.locals.lang = lang;
    req.locals.__ = res.locals.__ = (str, params) => strings.s('tpl', str, params, lang);
    req.locals.t = res.locals.t = (str, ...args) => t(str, ...args, {lang: req.locals.lang});
    next();
  });
...
// jshint maxstatements: 60, maxcomplexity: 60
  ...
  return load(path.join(rootPath, 'i18n'))
    ...
    .then((scope) => {
      ...
      return load(path.join(themeI18n, 'i18n'))
        .then(load(path.join(i18nAppDir, 'i18n')))
        .then(() => scope);
    })
