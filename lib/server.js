...
const {register} = require('core/i18n');
...
const t = msg => i18n.t(msg)({lang, domain: 'www'});
...
// TODO Access-лог надо реализовать через IonLogger
  ...
  app.use((req, res, next) => {
    ...
    for (locale of locales) {
      if (app.locals.supportedLanguages.includes(locale.language)) {
        lang = locale.language;
        break;
      }
    }
    req.locals.lang = lang;
    req.locals.__ = (str, params) => strings.s('tpl', str, params, lang);
    next();
  });
...
// jshint maxstatements: 60, maxcomplexity: 60
  ...
  return di('boot', config.bootstrap,
    ...
    .then((scope) => {
      ...
      return register(path.join(themeI18n, 'i18n'))
        .then(() => register(path.join(i18nAppDir, 'i18n')))
        .then(() => register(path.join(i18nAppDir, 'meta')))
        .then(() => scope);
    })
