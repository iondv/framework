...
const i18nSetup = require('core/i18n-setup').i18nSetupSync;
...
module.exports = (env) => {
  ...
  errorSetup(lang);
  i18nSetup(lang, config.i18n || 'i18n', 'i18n', sysLog);
...
// override with different headers; last one takes precedence
  ...
  app.locals.__ = (str, params) => strings.s('i18n', str, params);
...
// Связываем приложение
  return di('boot', config.bootstrap,
    ...
    .then((scope) => {
      ...
      i18nSetup(lang, themeI18n, 'i18n', sysLog);

      const i18nAppDir = scope.settings.get('i18n');
      i18nSetup(lang, i18nAppDir, 'i18n', sysLog);
      i18nSetup(lang, i18nAppDir && path.join(i18nAppDir, 'meta'), 'meta-captions', sysLog);
      return scope;
    })
    ...
    .then((scope) => {
      ...
      try {
        ...
        app.get('/api/i18n', dispatcher.api.i18n);
      } catch (err) {
