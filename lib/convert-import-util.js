...
const saveData = process.env.IMPORT.indexOf('DONTSAVE') === -1;
          ...
          for (let j = 0; j < dataToSave[className].length; j++) {
            ...
            try {
              ...
              /* Реализация проверки существования файла и обновление его - работает ДОЛГО
               // console.info('Сохраянем', fullFileNam);
               try {
              ...
              } finally { // Если файла нет, просто сохр
              ...
              console.error('Ошибка записи класса', className, 'файл', nameFile + '\n' + e);
              reject(e);
            }
          }
        }
        ...
        if (typeof updatedObject[key] !== 'undefined') {
          if (ignoreKeys.indexOf(key) === -1 && updatedObject[key] !== importedObject[key]) {
            if (Array.isArray(importedObject[key])) {
              ...
              /*console.log('При обновлении %s с id %s, заменяем для ключа %s значение "%s" на новое значение "%s"',
