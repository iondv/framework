...
const pathToPlatformData = path.join(__dirname, 'data'); // TODO переделать на именно путь к платформе, без application
      ...
      for (let className in dataToSave) {
        if (dataToSave.hasOwnProperty(className)) {
          for (let j = 0; j < dataToSave[className].length; j++) {
            ...
            try {
              // console.info('Сохраянем', path.join(pathToPlatformData, nameFile));
              // fs.writeFileSync(path.join(pathToPlatformData, nameFile),  // TODO переделать pathToPlatformData на путь платформы, добавление application и наймспейса из объекта
              //   JSON.stringify(dataToSave[className][j], null, 2), 'utf8');
              delete dataToSave[className][j]; // Очищаем память занимаемую объектом
              qntSavedObject++;
            } catch (e) {
              console.error('Ошибка записи класса', className + '\n' + e);
              reject(e);
            }
          }
        }
...
function verifyRequireValue(className, metaData, dataItemToConvert) {
  const ERR_STR_VAL = '!!!ОШИБШКА!!!, незаполненное обязательное поле';
  if (!metaData) {
    ...
    if (metaData.compositeIndexes) {
      ...
      metaData.compositeIndexes.forEach(function (indexItem, i) {
        ...
        if (indexItem.unique && indexItem.properties.length) {
          let indexKey = '';
          indexItem.properties.forEach(function (propIndexItem) {
            if (!dataItemToConvert[propIndexItem]) {
              for (let j = 0; j < metaData.properties.length; j++) {
                if (metaData.properties[j].name === propIndexItem) {
          ...
          if (metaData._indexValue[i][indexKey]) {
            ...
            console.warn('  Дублирование индекса класса', className, 'объекты', metaData._indexValue[i][indexKey]);
          } else {
            metaData._indexValue[i][indexKey] = dataItemToConvert[metaData.key[0]];
          }
        }
    ...
    const keyErr = 'Объект ' + dataItemToConvert[metaData.key[0]] + ' класса ' + className +
      ' отсутствует непустое значение ключа ' + metaData.key[0] + '. Попущен. Ключи ' + keysInData + '.' + util.inspect(dataItemToConvert, { showHidden: true, depth: 1});
    const indexErr = 'Объект ' + dataItemToConvert[metaData.key[0]] + ' класса ' + className +
      ' дублируется непустое значение индекса. Попущен.';
    const reqErr = 'Объект ' + dataItemToConvert[metaData.key[0]] + ' класса ' + className +
      ' отсутствует обязательное значение. Попущен.';
...
function getDirAndFilesList(pathInitDataFolder, param, done) {
  if (typeof pathInitDataFolder === 'function' && param === undefined) {
  ...
  } else if (typeof param === 'function' && done === undefined) {
    done = param;
    param = {
      recurse: false,
      type: 'full'
