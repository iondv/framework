...
const SAVE_DATA = process.env.IMPORT ? process.env.IMPORT.indexOf('DONTSAVE') === -1 : true;
console.log('Статус сохранения данных', SAVE_DATA);
 ...
 * @param {Array} skipClassName  - массив названия файлов, для пропуска сохранения и удаления
 * @returns {Promise}
 */
function saveImportedFiles(importedData, skipClassName) {
  skipClassName = skipClassName || [];
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      let zip = new JSZip();
      let zipIgnore = importedData.config.zipIgnore;
      const SAVE_ZIP =  importedData.config.zip;
      for (let className in dataToSave) {
        if (dataToSave.hasOwnProperty(className) && skipClassName.indexOf(className) === -1) {
          for (let j = 0; j < dataToSave[className].length; j++) {
            if (dataToSave[className][j]) {
              ...
              try {
                ...
                if (SAVE_DATA) {
                  if (SAVE_ZIP && (!zipIgnore || nameFile.search(zipIgnore) === -1)) {
                    zip.file(nameFile, JSON.stringify(objData, null, 2));
                  } else {
      ...
      if (SAVE_ZIP) {
        zip.generateAsync({type: 'nodebuffer'})
          .then(function (content) {
            let  zipFileName =  path.join(importedData.pathData, 'import-data_' + zipIteration + '.zip');
            zipIteration++;
            fs.writeFile(zipFileName, content, function (err) {
              if (!err) {
                console.info('Сохранили импорт в архив', zipFileName);
                resolve(importedData);
              } else {
                console.error('Ошибка записи архива', zipFileName + '\n' + err);
                reject(e);
              }
            });
          });
...
/**
 * Обновление импортируемых объектов
 * @param {Object} importedData - объект с данными импорта
 * @param {Object} importedData.meta - структура метаданных, наименование объекта - класс с неймспейсом: adminTerritory@khv-svyaz-info
 * @param {Object} importedData.result  - именованный массивом объектов - имя с названием класса, значения - массив данных импорта
 * @returns {Promise}
 */
function updateImportedFiles(importedData) {
  let metaData = importedData.meta;
  let dataToSave = importedData.result;
  return new Promise(function (resolve, reject) {
    try {
          ...
          for (let j = 0; j < dataToSave[className].length; j++) {
            if (dataToSave[className][j]) {
              ...
              try {
                ...
                // console.info('Обновляем', nameFile);
                try {
                  ...
                  // console.info('НЕТ ФАЙЛА для обновления', nameFile);
                } finally { // Если файла нет, просто сохраняем
                  if (SAVE_DATA) {
                    fs.writeFileSync(fullFileName, JSON.stringify(objData, null, 2), 'utf8');
                    // 4debug console.log('Сохранён обновленный файл', nameFile);
                    qntSavedObject++;
                  }
                }
              } catch (e) {
              ...
              delete dataToSave[className][j]; // Очищаем память занимаемую объектом, не чистим раньше, т.к. в функции setObjectFileName при прогоне удаляется, раньше, чем сохраняется.            }
            }
          }
         /* for (let j = 0; j < dataToSave[className].length; j++) {
            delete dataToSave[className][j]; // Очищаем память занимаемую объектом, не чистим раньше, т.к. в функции setObjectFileName при прогоне удаляется, раньше, чем сохраняется.
            dataToSave[className][j] = null;
          }*/
          console.info('Обновлено файлов %s, из объектов для обновления %s класса %s', qntSavedObject,
            dataToSave[className].length, className);
        }
      }
...
module.exports.updateImportedFiles = updateImportedFiles;
      ...
      if (importedObject.hasOwnProperty(key)) {
        if (typeof updatedObject[key] !== 'undefined') {
          if (ignoreKeys.indexOf(key) === -1 && updatedObject[key] !== importedObject[key]) {
            ...
            } else { // TODO можно добавить сравнение объектов и их свойств на любую глубину
 ...
 * @param {Object} targetObject сравниваемый объект
 * @param {Object} sourceObject - объект, с которым сравниваем
 * @param {Array} ignoreKeys - игнорируемые ключи объекта
 * @param {String} className - имя класса обновляемого объекта
 * @returns {Boolean} resComare результат сравнения
 */
function compareObjects(targetObject, sourceObject, ignoreKeys, className) {
  let resCompare = true;
  if (!ignoreKeys) {
    ignoreKeys = [];
  }
  ...
  if (typeof targetObject !== 'undefined') {
    for (let key in sourceObject) {
      if (sourceObject.hasOwnProperty(key)) {
        if (typeof targetObject[key] !== 'undefined') {
          if (ignoreKeys.indexOf(key) === -1 && targetObject[key] !== sourceObject[key]) {
            if (Array.isArray(sourceObject[key])) {
              for (let i = 0; i < sourceObject[key].length; i++) {
                if (targetObject[key].indexOf(sourceObject[key][i]) === -1) {
                  resCompare = false;
                  break;
                }
              }
            } else { // TODO можно добавить сравнение объектов и их свойств на любую глубину
              if (targetObject[key] !== sourceObject[key]) { // TODO неправильно сравнивает даты, например созданные из внешних БД - по ISO одинаковые, при сравнеии разные
                resCompare = false;
              }
            }
          }
    ...
    console.warn('Осутствует сравниваемый объект', className, sourceObject);
    resCompare =  false;
  }
  return resCompare;
}
  ...
  if (!metaData) {
    ...
    metaData.key.forEach((item) => {
      try {
        ...
        console.error('Ошибка установления имения файла класса', className, metaData.key, dataToConvert, e, e.stack);
      }
    });
    return className + '@' + idFromKey + '.json';
  }
...
/**
 * Функция сравнения двух строк напредмет опечаток и отличий на несколько символов. Если пропуск буквы - то будет больше расхождений
 * @param {String} string1 - первая строка для сравнения
 * @param {String} string2 - вторая строка для сравнения
 * @param {Number} qntMisspell
 * @returns {Number} значение кол-ва расхождений если строки отличаются меньше чем на qntMisspell значения, -1 - если на большее
 *
 */
function searchMisspell(string1, string2, qntMisspell) {
  if (typeof string1 !== 'string' || typeof string2 !== 'string' || qntMisspell < 1) { // Не строки - нечего сравниваться, меньше одной ошибки нет смысла искать
    return -1;
  }
  // Если длина строк отличается больше чем на кол-во возможных ошибок
  let diffStr = string1.length > string2.length ? string1.length - string2.length : string2.length - string1.length;
  if (diffStr > qntMisspell) {
    // 4debug console.log('= Строки различаются большей чем на кол-во ошибок');
    return -1;
  }
  // Длина строк меньше чем кол-во ошибок
  if (string1.length < qntMisspell || string2.length < qntMisspell) {
    // 4debug console.log('= Длина одной из строк, меньше кол-во ошибок');
    return -1;
  }
  // Если строки равны, дальше сравнивать не нужно
  if (string1.toLowerCase() === string2.toLowerCase()) {
    // 4debug console.log('= Строки одинаковы');
    return 0;
  }
  if (string1.length > string2.length) { // Делаем string2 - длиннее, предполагая что в первой пропуске
    let tmpString = string1;
    string1 = string2;
    string2 = tmpString;
  }
  let qntNotEqualent = 0; // Кол-во ошибок
  let regExpString = '';
  for (let i = 0; i < string1.length; i++) {
    regExpString +=  ['(',')','?','*','^','[',']','+','{','}','$','\\','|',':','.'].indexOf(string1[i]) === -1 ?
      string1[i] + '?' : '\\' + string1[i] + '?'; // Исключаем спец.символы regexp и формируем возможность замены символа

  }
  let arrNotEqualent = string2.match(new RegExp(regExpString, 'ig'));
  // 4debug console.log('Сравниваемые строки', string1, string2, arrNotEqualent);
  let qntNull = 0;
  for (let i = 0; i < arrNotEqualent.length; i++) {
    if (arrNotEqualent[i] === '') {
      qntNull++;
    }
  }
  qntNotEqualent = arrNotEqualent.length === qntNull ? // Если не найденно значений
                     arrNotEqualent.length - 1 : // Иначе кол-во равно
                     arrNotEqualent.length - 2;

  if (qntNotEqualent > 0) {
    // 4debug console.log('= Ошибочных букв', arrNotEqualent, qntNotEqualent);
    if (qntNotEqualent > qntMisspell) {
      return -1;
    } else {
      return qntNotEqualent;
    }
  }

  return qntNotEqualent;
}
module.exports.searchMisspell = searchMisspell;

/**
 * Функция проверки опечаток среди всех значений двух массивовв, сопоставляя их поэлементно.
 * Удобно использовать для проверки опечаторк в ФИО и ДР персон
 * Пример проверки console.log('#', checkMisspelInArray(['БРЕСЛАВЕЦ', 'ИРИНА', 'МИХАЙЛОВНА', '1982-01-01'],
 * ['БРЕСЛАВЕЦ', 'АРИНА', 'МИХАЙЛОВНА', '1982-01-01'], 2));
 * @param {Array} arr1 - массив строк для сравнения
 * @param {Array} arr2 - массив строк для сравнения
 * @param {Number} qntMisspell - кол-во ошибок, после которых останавливается проверка и возвращается значение -1 о превышении
 * @param {Number} index - индекс проверяемого элемента
 * @param {Number} qntFindedMisspell - кол-во найденных ошибок, в предыдущих итерациях
 * @returns {*}
 */
function checkMisspelInArray (arr1, arr2, qntMisspell, index, qntFindedMisspell) {
  if (!Array.isArray(arr1) || !Array.isArray(arr2) || arr1.length !== arr2.length || arr1.length < 1) { // Если массивы разной длины или без элементов, не понятно что сравнивать - очевидно разные
    return -1;
  }
  if (typeof index !== 'undefined' && typeof qntFindedMisspell === 'undefined') { // Ошибочный вызов рекурсивной функции
    throw new Error ('Нерпавильный вызов функции checkMisspelInArray'); // Или оба незаданы параметра, или оба заданы
  }
  qntFindedMisspell = typeof qntFindedMisspell === 'undefined' ? 0 : qntFindedMisspell;
  index = typeof index === 'undefined' ? 0 : index;

  if (arr1[index] && arr2[index]) { // Незаданные значения пропускаем из сравнения
    let firstMisspell = searchMisspell(arr1[index], arr2[index], qntMisspell);
    qntFindedMisspell += firstMisspell === -1 ? qntMisspell + 1 : firstMisspell; // Если результат сравнения -1 - значит ошибок больше чем минимум
  }
  if (qntFindedMisspell <= qntMisspell && arr1.length > index + 1) {
    index++;
    qntFindedMisspell = checkMisspelInArray(arr1, arr2, qntMisspell, index, qntFindedMisspell);
  }
  if (qntFindedMisspell <= qntMisspell) {
    return qntFindedMisspell;
  }
  return -1;
}

module.exports.checkMisspelInArray = checkMisspelInArray;

/**
 * Функция замены похожи англоязычных русским символам
 * @param {String} str
 * @returns {*}
 */
function replaceEngEqualRusSymbol(str) {
  [['A', 'А'], ['B', 'В'], ['O', 'О'], ['P', 'Р'], ['X', 'Х'], ['E', 'Е'], ['T', 'Т'], ['H', 'Н'], ['K', 'К'],
    ['C', 'С'], ['M', 'М'], ['a', 'а'], ['o', 'о'], ['e', 'е'], ['c', 'с'], ['p', 'р'], ['k', 'к'], ['x', 'х'],
    ['u', 'и'], ['b', 'ь']]
  .forEach(item => {
    str = str.replace(new RegExp(item[0], 'g'), item[1]);
  });
  if (str.search(/[A-Za-z]/) !== -1) {
    console.warn('Не все английские символы заменены русскими в ', str);
  }
  return str;
}
module.exports.replaceEngEqualRusSymbol = replaceEngEqualRusSymbol;
          ...
          if (!importedData.meta[importClassName]) {
            ...
            for (let i = 0; i < importedReference[importClassName].length; i++) {
              ...
              if (!foundedExistingObj) {
                if (typeof importedReference[importClassName][i]._class === 'undefined') {
                  importedReference[importClassName][i]._class = importClassName;
                }
                if (typeof importedReference[importClassName][i]._classVer === 'undefined') {
                  importedReference[importClassName][i]._classVer = '';
                }
