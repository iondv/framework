      ...
      for (let className in dataToSave) {
        if (dataToSave.hasOwnProperty(className)) {
          for (let j = 0; j < dataToSave[className].length; j++) {
            ...
            try {
              // console.info('Сохраянем', path.join(importedData.pathData, nameFile));
              // TODO !!!!!!!!!!! проверять файл, если он уже есть, то мы его не перетираем, а считываем и обновляем. Обновление такое - все поля, кроме массиов - перетираем, массивы проверяем элементы, если уже есть значение, сохраняем, нет добавляем элемент
              // fs.writeFileSync(path.join(importedData.pathData, nameFile),  // TODO переделать pathToPlatformData на путь платформы, добавление application и наймспейса из объекта
...
function verifyRequireValue(className, metaData, dataItemToConvert) {
  ...
  if (!metaData) {
    ...
    let sliceInfo = process.env.DEBUG === 'SILENCE' ? true : false;
      ...
      metaData.compositeIndexes.forEach(function (indexItem, i) {
        ...
        if (indexItem.unique && indexItem.properties.length) {
          ...
          if (metaData._indexValue[i][indexKey]) {
            ...
            if (!sliceInfo) {
              console.warn('  Дублирование индекса', indexKey, 'класса', className, 'объекты',
              metaData._indexValue[i][indexKey]);
            }
