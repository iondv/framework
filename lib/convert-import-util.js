      ...
      for (let className in dataToSave) {
        if (dataToSave.hasOwnProperty(className) && skipClassName.indexOf(className) === -1) {
          for (let j = 0; j < dataToSave[className].length; j++) {
            let nameFile = setObjectFileName(className, metaData[className], dataToSave[className][j], dataToSave[className].length, j);
          ...
          for (let j = 0; j < dataToSave[className].length; j++) {
            ...
            try {
              ...
              try {
                ...
                if (SAVE_DATA) {
                  fs.writeFileSync(fullFileName, JSON.stringify(objData, null, 2), 'utf8');
                  console.log('Сохранён обновленный файл', nameFile);
                  qntSavedObject++;
                }
              }
              delete dataToSave[className][j]; // Очищаем память занимаемую объектом
 ...
 */
function setObjectFileName(className, metaData, dataToConvert, len, j) {
  let idFromKey = '';
  if (!metaData) {
    ...
    metaData.key.forEach((item) => {
      try {
        ...
        console.error('dataToConvert', dataToConvert);
        console.error('className len j', className, len, j);
        process.exit(0);
      }
    });
    return className + '@' + idFromKey + '.json';
