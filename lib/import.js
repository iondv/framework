  ...
  var meta = function () {
    ...
    processDir(
      ...
      function (fn) {
        ...
        metas.push(cm);
        mByName[`${cm.name}@${cm.namespace}`] = cm;
      }
    );

    function defineClass(cm) {
      return function () {
        return sync.defineClass(cm);
      };
    }
    ...
    function calcClassLevel(cm) {
      if (cm.ancestor) {
        ...
        if (mByName.hasOwnProperty(cm.ancestor)) {
          ...
          anc = metaRepo.getMeta(cm.ancestor, cm.version || null);
          if (anc) {
            anc = anc.plain;
          }
        }
    ...
    processDir(
      ...
      function (fn) {
        if (fn.substr(-13) === '.section.json') {
          ...
          promises.push(sync.defineNavSection(s));
        } else {
          var n = JSON.parse(fs.readFileSync(fn), {encoding: 'utf-8'});
          var pth = path.parse(fn);
          promises.push(sync.defineNavNode(n, path.relative(nd, pth.dir)));
        }
      }
    );
    if (promises.length) {
  ...
  var workflows = function () {
    ...
    processDir(
      ...
      function (fn) {
        ...
        promises.push(sync.defineWorkflow(wf));
      }
    );
    if (promises.length) {
      return Promise.all(promises);
