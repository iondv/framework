...
module.exports = function (src, sync, metaRepo, dataRepo, options) {
   ...
   */
  var data = function () {
    const rangeToSave = 1000; // Нужно уменьшить, если при импорте объектов выдается ошибка нехватки памяти или нехватки старой памяти JavaScript heap out of memory
    /**
    ...
    function saveIterationOfData(filesList, start) {
      ...
      return Promise.all(savePromises)
        .then((res) => {
          savePromises.forEach((item) => { // Удаляем объекты - для очистки занимаемой объектами памяти
            delete item;
          });
