...
module.exports = function (src, options) {
    ...
    if (fs.existsSync(wfd)) {
      processDir(
        ...
        (fn) => {
          ...
          wfNames[(wf.namespace || options.namespace) + '@' + wf.name] = true;
  ...
  function defineWorkflowView(fn, parts) {
    ...
    return () => options.sync.defineView(
      ...
      'workflows:' + parts.join('.') // Путь навигации
    );
  }
          ...
          try {
            ...
            if (parts[0] === 'workflows' && parts.length === 3) {
              ...
              if (!navNodes[navNodeCode] && !options.metaRepo.getNode(navNodeCode, options.namespace)) {
                let wfName = parts[1];
                if (wfName.indexOf('@') < 0) {
                  wfName = options.namespace + '@' + wfName;
                }
                if (wfNames[wfName] || options.metaRepo.getWorkflow(pth.name, wfName, options.namespace)) {
                  isWfV = true;
                  (options.log || console).warn(
                    `Модель представления для статуса бизнес-процесса ${wfName}.${parts[2]}` +
                    ' расположена в директории "workflows" вместо "wfviews".' +
                    ' Этот способ является устаревшим и будет исключен в следующих версиях.'
                  );
                } else {
                  (options.log || console).warn('Не удалось интерпретировать путь модели представления ' + fn + '!');
                  return;
                }
              }
            }
