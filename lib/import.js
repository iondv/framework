...
module.exports = function (src, sync, metaRepo, dataRepo, log, options) {
  ...
  var data = function () {
    ...
    function itemSaver(className, id, obj) {
      return dataRepo.saveItem(
        ...
        .catch((err) => {
          log.warn(`Пропускаем импорт объекта ${className}@${id}, из-за ошибки: ${err.message}`);
    ...
    function saveIterationFileList(filesList, start) {
      ...
      for (var i = start; i < finish; i++) {
        let obj = JSON.parse(fs.readFileSync(filesList[i], {encoding: 'utf-8'}), fileParser);
