...
module.exports = function (src, sync, metaRepo, dataRepo, options) {
  ...
  var dataMaxMemmori = function () { // 2del К удалению - предыдущий вариант импорт - для большого кол-ва объектов, более 8 тыс. вызывала нехватку памяти 1,4 Гб
    var promises = [];
    processDir(
      dd,
      function (nm) {
      ...
      return new Promise(function (resolve, reject) {
        metaRepo.init().then(function () {
          ...
          for (var i = 0; i < promises.length; i++) {
            tmp.push(promises[i]());
            delete promises[i]; // Освобождаем память
          }
          promises = null;
  ...
  /**
   * Импорт данных из папки data приложения
   * @returns {Promise} - возвращает результат promise всех записей, или catch первой сбойной
   */
  var data = function () {
    const rangeToSave = 100; // Нужно уменьшить, если при импорте объектов выдается ошибка нехватки памяти или нехватки старой памяти JavaScript heap out of memory
    /**
     ...
     */
    function saveIterationOfData(filesList, start) {
      start = start ? start : 0;
      let savePromises = [];
      let finish = start + rangeToSave < filesList.length ?  start + rangeToSave : filesList.length;
      ...
      return Promise.all(savePromises)
        .then((res) => {
          // Не помогает - в докере все равно сыпется на том же объме, что и просто обнулениеsavePromises.forEach((item) => { // Удаляем объекты - для очистки занимаемой объектами памяти
          //   delete item;
          // });
          ...
          if (finish === filesList.length || finish / 1000 === Math.round(finish / 1000)) { // Показываем только тысячи импорированных данных или финиш импорта
            console.info('Импортированаы партии по %s объектов данных. Всего %s/%s', rangeToSave, finish,
              filesList.length);
          }
          if (start + rangeToSave < filesList.length) {
            return saveIterationOfData(filesList, start + rangeToSave);
          } else {
            return res;
          }
        })
    ...
    console.info('Кол-во импортируемых объектов данных', filesList.length);
    if (filesList.length) {
      return new Promise(function (resolve, reject) {
        metaRepo.init()
          .then(function () {
            return saveIterationOfData(filesList, 0);
          })
          .then(resolve)
          .catch(reject);
      });
