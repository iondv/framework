...
module.exports = function (src, sync, metaRepo, dataRepo, options) {
   ...
   */
  var data = function () {
    const rangeToSave = 1000; // Нужно уменьшить, если при импорте объектов выдается ошибка нехватки памяти
    /**
    ...
    function saveIterationOfData(filesList, start) {
      ...
      return Promise.all(savePromises)
        .then((res) => {
          savePromises = null; // Обнуляем для очистки занимаемой объектами памяти
          console.info('Импортирована партия данных из %s объектов [%s/%s]', rangeToSave, finish, filesList.length);
