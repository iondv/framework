      ...
      processDir(
        ...
        (fn) => {
          try {
            ...
            //let type = parts2[0];
            if (parts[0] === 'workflows' && !['create', 'item', 'list'].includes(pth.name)) {
              (options.log || console).warn(`Модели представлений для статусов бизнесс-процесов ` + 
                `расположенны в директории ${path.join(vd, 'workflows')} вместо ${wfvd}.` + 
                ` Этот формат является устаревшим и будет исключен в следующих версиях.`);
              //{
              let cn = pth.name;
              ...
              promise = promise.then(() => options.sync.defineView(
                vm, // Объект модели представления
                cn, // Имя класса
                pth.name/*type*/, // Тип представления
        ...
        (fn) => {
          try {
            ...
            let cn = pth.name;
              ...
              if (parts[0].indexOf('@') < 0 && options.namespace) {
