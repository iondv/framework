  ...
  var data = function () {
    ...
    function saveIterationFileList(filesList, start) {
      ...
      for (var i = start; i < finish; i++) {
        ...
        let saveWithErrSkip = new Promise(function (resolve, reject) {
          return dataRepo.saveItem(
            ...
            .then(resolve)
            .catch((err) => {
              console.warn('Пропускаем импорт объекта, из-за ошибки:', err.message);
              resolve();
            });
        });
      ...
      return Promise.all(savePromises)
        ...
        .catch((err) => {
          console.warn('Ошибка импорта файл %s. Дальнейший импорт прекращен. Ошибка:\n%s', filesList[i], err.message,
            '\n', err.stack);
          return err;
        });
    }
    ...
    function saveIterationZip(zipList, start, numZip, zipData) {
      ...
      if (!zipData) {
        ...
        return Promise.all(savePromises)
          ...
          .catch((err) => {
            console.error('Ошибка сохранения партии объектов', zipList[numZip], savePromises.length, start, finish, err);
            return err;
          });
      }
    }
