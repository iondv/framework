  ...
  var data = function () {
    ...
    function saveIterationFileList(filesList, start) {
      ...
      for (var i = start; i < finish; i++) {
        ...
        let parts = path.parse(decodeURIComponent(filesList[i])).name.split('@');
        let saveWithErrSkip = new Promise(function (resolve, reject) {
          return dataRepo.saveItem(
            parts[0] + (options.namespace ? '@' + options.namespace : ''),
            parts.length > 2 ? parts[2] : parts.length > 1 ? parts[1] : null,
            obj, obj._classVer || null, null, {skipResult: true, nestingDepth: 0, autoAssign: true,
              ignoreIntegrityCheck: options.ignoreIntegrityCheck || false})
            .then(resolve)
            .catch((err) => {
              log.warn(`Пропускаем импорт объекта, из-за ошибки: ${err.message}`);
              resolve();
            });
        });
        savePromises.push(saveWithErrSkip); // Формируем массив с promise для запуска сохранения
      }
      return Promise.all(savePromises)
        .then((res) => {
          ...
          if (finish === filesList.length || finish / 1000 === Math.round(finish / 1000)) { // Показываем только тысячи импорированных данных или финиш импорта
            log.info(`Импортирована партия по ${SAVE_RANGE} объектов данных. Всего ${finish}/${filesList.length}`);
          }
          if (finish < filesList.length) {
            return saveIterationFileList(filesList, finish);
          } else {
            return res;
          }
        })
        .catch((err) => {
          log.warn(`Ошибка импорта файл ${filesList[i]}. Дальнейший импорт прекращен. 
            Ошибка: ${err.message} ${err.stack}`);
          return err;
        });
    }
    ...
    function setSavePromise(filename, zipData) {
      return zipData
        ...
        .catch ((err)=> {
          log.error(`Ошибка распаковки файла ${filename}`);
          return err;
        });
    }
    ...
    function saveIterationZip(zipList, start, numZip, zipData) {
      ...
      if (!zipData) {
        ...
        return zip.loadAsync(data)
          .then((res) => {
            zipData = res;
            log.log(`Считали архив ${zipList[numZip]}`);
        ...
        return Promise.all(savePromises)
          .then((res) => {
            ...
            savePromises = null; // Очищаем массив
            if (finish === filesList.length || finish / 1000 === Math.round(finish / 1000)) { // Показываем только тысячи импорированных данных или финиш импорта
              log.info(`Импортирована партия по ${SAVE_RANGE} объектов данных из архива ${zipList[numZip]}. 
              Всего ${finish}/${filesList.length}`);
            }
            if (finish < filesList.length) {
          ...
          .catch((err) => {
            log.error(`Ошибка сохранения партии объектов 
            ${zipList[numZip]} ${savePromises.length} ${start} ${finish} ${err}`);
            return err;
          });
      }
    }
