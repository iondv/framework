...
module.exports = function (src, sync, metaRepo, dataRepo, log, options) {
  ...
  function meta() {
    ...
    metas.forEach((m) => {
      ...
      promiseLevels['l' + lvl].promises.push(() => sync.defineClass(m, options.namespace));
    });
        ...
        (fn) => {
          try {
            ...
            if (parts[0] === 'workflows') {
              ...
              promise = promise.then(() => sync.defineView(
      ...
      processDir(
        ...
        (fn) => {
          if (fn.substr(-13) === '.section.json') {
            ...
            promise = promise.then(() => sync.defineNavSection(s, options.namespace));
          } else {
            let n = JSON.parse(fs.readFileSync(fn), {encoding: 'utf-8'});
            let pth = path.parse(fn);
            promise = promise.then(() => sync.defineNavNode(n, path.relative(nd, pth.dir), options.namespace));
          }
        },
        (e) => {
          log.error(e);
    ...
    if (fs.existsSync(wfd)) {
      processDir(
        ...
        (fn) => {
          ...
          promise = promise.then(() => sync.defineWorkflow(wf, options.namespace));
        },
        (err) => {
          log.error(err);
        }
  ...
  function userTypes() {
    ...
    if (fs.existsSync(utd)) {
      processDir(
        ...
        (fn) => {
          let s = JSON.parse(fs.readFileSync(fn), {encoding: 'utf-8'});
          promise = promise.then(() => sync.defineUserType(s));
        },
        (err) => {
          log.error(err);
        }
  ...
  return sync.init()
    ...
    .then((res) => {
      if (options.skip && options.skip.indexOf('data') !== -1) {
        log.info('Пропущен импорт данных в БД');
        return res;
      } else {
        return data(res);
      }
