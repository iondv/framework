  ...
  var server = new OAuth2Server({
    ...
    model: {
          ...
          .fetch('ion_oauth2_token',
            {
              filter: {
                [F.AND]: [
                  ...
                  {[F.GREATER_OR_EQUAL]: ['$accessTokenExpiresOn', new Date()]}
                ]
              }
            }
          )
          ...
          .fetch('ion_oauth2_token',
            {
              filter: {
                [F.AND]: [
                  ...
                  {[F.GREATER_OR_EQUAL]: ['$refreshTokenExpiresOn', new Date()]}
                ]
              }
            }
          )
      ...
      saveToken: function (token, client, user) {
        ...
        return options.dataSource
          .delete('ion_oauth2_token', {[F.LESS]: ['$expires', new Date()]})
...
function handleError(e, req, res, response) {
  ...
  res.status(400);
  res.send({code: e.code, error: e.name, error_description: e.message});
}
