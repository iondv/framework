    ...
    return dataRepo
      ...
      .then((list) => {
        ...
        if (total % 100 == 0 || list.length < step) {
          promise = promise.then(() => log && log(`Processed ${total} objects`));
        }
        if (list.length === step) {
          promise = promise
    ...
    classList.forEach((cn) => {
      ...
      cachedClasses.forEach((cn) => {
        classPromise = classPromise.then(() => {
          log && log(`Recalculation for objects of ${cn} class`);
          return recacheChunk(dataRepo, cn, {skipCacheDependencies: skipDep}, log, {step, sort});
        });
      });
    });
    return classPromise.then(() => log && log('Recount completed!'));
  } catch (e) {
