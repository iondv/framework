...
module.exports = function (app, moduleName, baseDir, themePath, log) {
  ...
  if (!themePath) {
    ...
    if (fs.existsSync(path.join(themePath, 'vendor'))) {
      app.use('/' + moduleName + '/vendor', express.static(path.join(themePath, 'vendor')));
    }
    var favico = path.join(themePath, 'static', 'favicon.ico');
