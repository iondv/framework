...
Strategy.prototype.authenticate = function (req, options) {
  ...
  if (req.path.slice(-8) === 'callback') {
    ...
    let post = {
      method: 'POST',
      uri: join(_this.portalUrl, _this.tokenUrl),
      form: {
    ...
    requester(post)
      .then(result => {
        ...
        let personInfoUrl = join(_this.portalUrl, _this.personUrl, oid);
    ...
    let query = {
      timestamp,
      client_id: _this.mnemonik,
      redirect_uri: join(currentUrl(req), _this.redirectUrl),
      scope: _this.scope,
      response_type: 'code',
      state,
      access_type: 'online',
    ...
    let authUrl = join(_this.portalUrl, _this.codeUrl) + '?' + querystring.stringify(query);
