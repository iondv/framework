...
function requestContactsInfo(portalUrl, params, oid, token) {
  return requester({
      uri: url.resolve(portalUrl, join('rs/prns/${oid}/ctts')),
      auth: {bearer: token}
    })
    .then(res => {
      let promises = [];
...
function EsiaStrategy(options, verify) {
  ...
  this.scope = 'fullname birthdate gender snils inn email';
    ...
    requester(post)
      .then(result => {
        ...
        return scopeDataRequester(_this.portalUrl, oid, _this.scope, token)
          .then(p => {
            console.log(oid, token);
            let profile = {
              oid: String(oid)
            };
            let rrr = Object.assign(profile, p);
            console.log(p);
            console.log(rrr);
            return rrr;
          });
