...
function pkcsEncrypt(message, cer, key) {
  try {
    const certOrCertPem = fs.readFileSync(cer, {encoding: 'utf-8'}); // eslint-disable-line
    const keyAssociatedWithCert = fs.readFileSync(key, {encoding: 'utf-8'}); // eslint-disable-line
  ...
  if (req.path.slice(-8) === 'callback') {
    ...
    return requester(post)
      .then((result) => {
        ...
        const pk = fs.readFileSync(_this.tokenPk); // eslint-disable-line
