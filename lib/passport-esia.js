...
function scopeGetter(scopeType, params, portalUrl, oid, token) {
  switch (scopeType) {
    case 'person':
      return requestPersonInfo(portalUrl, params, oid, token);
    case 'contacts':
      return requestContactsInfo(portalUrl, params, oid, token);
    default:
 ...
 * @return {Promise}
 */
function scopeDataRequester(oid, scope, token) {
  let _this = this;
  let properties = {};
  ...
  if (hasScopes(_this.scope, scopeTypes.PERSON)) {
    promises = promises
      .then(() => infoRequester(url.resolve(_this.portalUrl, join('rs/prns', String(oid))), token, _this.personInfoSchema))
      .then((result) => {
...
EsiaStrategy.prototype.authenticate = function (req) {
  ...
  if (req.path.slice(-8) === 'callback') {
    ...
    sendAccessTokenRequest.call(_this, req.query.code, at_state, url.resolve(currentUrl(req), _this.redirectUrl))
      ...
      .then(profile => _this._verify(Object.assign({oid: String(oid), properties: profile}, profile), verified))
      .catch(err => _this.error(err));
  } else {
    const state = uuidv4();
    req.session.esiaState = state;
    sendAuthCodeRequest.call(_this, state, url.resolve(currentUrl(req), this.redirectUrl)).catch(err => _this.error(err));
  }
};
...
function sendAccessTokenRequest(auth_code, state, redirect_uri) {
  ...
  return this._encrypt(msg).then((secret) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      }, (err, req, body) => err ? reject(err) : resolve(body));
    });
  });
}
