...
EsiaStrategy.prototype.authenticate = function (req) {
  ...
  if (req.path.slice(-8) === 'callback') {
    ...
    let at_state = uuidv4();
    sendAccessTokenRequest.call(_this, req.query.code, at_state, url.resolve(currentUrl(req), _this.redirectUrl)).then(
      function (err, req, body) {
        ...
        if (at_state !== result.state) {
          return _this.fail('Идентификатор запроса к ЕСИА был нарушен');
        }

        const token = result.access_token;
        const refreshToken = _this.access_type === 'offline' ? result.refresh_token : null;
