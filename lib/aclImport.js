...
module.exports = function (aclDir, accessManager, log, auth) {
  if (accessManager instanceof RoleAccessManager) {
    if (fs.existsSync(aclDir) && fs.statSync(aclDir).isDirectory()) {
        ...
        if (fs.statSync(f).isFile()) {
          chain1 = chain1.then(() => {
            if (path.extname(f) === '.yml') {
              ...
              return Promise.reject(`File ${f} not read. Unknown extension.`);
            }
          }).then((data) => {
            //TODO
            console.log(data);
      ...
      return Promise.reject(new Error(`Acl directory does not exists.`));
    }
  } else {
...
function aclObjectImport(object, am, log, auth) {
  switch (object.type) {
    case 'resource':
      return am.defineResource(object.id, object.name).catch((err) => log.error(err));
    case 'role':
      return am.defineRole(object.id, object.caption)
