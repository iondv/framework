  ...
  if (accessManager instanceof RoleAccessManager) {
    if (fs.existsSync(aclDir) && fs.statSync(aclDir).isDirectory()) {
      ...
      files.forEach(function (file) {
        ...
        if (stat.isFile() && (path.extname(f) === '.yml' || path.extname(f) === '.json')) {
        ...
        } else if (stat.isDirectory() && ['resources', 'roles', 'users'].includes(file)) {
          const dirFiles = fs.readdirSync(f);
          dirFiles.forEach((res) => {
            const resPath = path.join(f, res);
            const resStat = fs.statSync(resPath);
            if (resStat.isFile()) {
              chain = chain.then(() => path.extname(res) === '.yml' ? readYAML(resPath) : readJSON(resPath))
                .then((results) => {
                  let promise = Promise.resolve();
                  results = Array.isArray(results) ? results : [results];
                  results.forEach((data) => {
                    if (file === 'resources') {
                      promise = promise.then(() => importResource(data, accessManager, log));
                    } else if (file === 'roles') {
                      promise = promise.then(() => importRole(data, accessManager, log));
                    } else if (file === 'users') {
                      promise = promise.then(() => importUser(data, auth, accessManager, log));
                    }
                  });
                  return promise;
                })
                .then(() => log.info(`Файл ${resPath} импортирован.`))
                .catch((err) => {
                  log.error(`Файл ${resPath} не был импортирован.`);
                  log.error(err);
                });
            }
          });
