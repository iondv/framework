...
function isBefore({value=5, unit='m'}, time2) {
  let time1 = moment().subtract(value, unit);
  if (time2) {
...
// jshint maxstatements: 200, maxcomplexity: 200
/**
...
function Auth(options) {
  ...
  var passwordLifetime = parseDuration(options.passwordLifetime || '120d');
  var passwordMinLength = options.passwordMinLength || 6;
  var failLoginsCount = options.failLoginsCount || 3;
  var tempBlockTimeOut = parseDuration(options.tempBlockTimeOut || '1m');
  var tempBlockCount = options.tempBlockCount || 3;
  var tempBlocksPeriod = parseDuration(options.tempBlocksPeriod || '10m');
  var blockTimeOut = parseDuration(options.blockTimeOut || '1m');
  ...
  function upsertFail(username, fail) {
    ...
    if (!fail) {
      ...
      if (update.tmpBlocks >= tempBlockCount) {
        if (isBefore(tempBlocksPeriod, fail.firstTmpBlock)) {
  ...
  function isBlocked(fail) {
    ...
    if (fail) {
      if (fail.lastBlock) {
        if (isBefore(blockTimeOut, fail.lastBlock)) {
          blocked = true;
        } else {
          return _this.ds.delete('ion_sec_fail_login', {username: {$eq: fail.username}})
            .then(() => {return {blocked, fail: null};});
        }
      }
      if (!blocked && fail.lastTmpBlock) {
        if (isBefore(tempBlockTimeOut, fail.lastTmpBlock)) {
          blocked = true;
        } else {
          let upd = {attempts: null, lastAttempt: null, lastTmpBlock: null};
          return _this.ds.update('ion_sec_fail_login', {username: {$eq: fail.username}}, upd)
            .then(fail => {return {blocked, fail};});
        }
      }
    }
    return {blocked, fail};
              ...
              if (users.length > 0) {
                ...
                checker.verifyAgainst(users[0].pwd, function (err, verified) {
                  if (verified) {
                    ...
                    return upsertFail(username, fail)
  ...
  function livePassword({pwdDate}) {
    return isBefore(passwordLifetime, pwdDate);
  }
