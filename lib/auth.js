  ...
  function upsertFail(username, fail) {
    if (!fail) {
      ...
      if (update.attempts >= failLoginsCount) {
        ...
        update.lastBlock = new Date();
      }
      return _this.ds.update('ion_sec_fail_login', {username: {$eq: username}}, update);
    }
  }

  function upsertBlock(username, fail) {
    let data = {
      blocks: fail && fail.blocks ? fail.blocks + 1 : 1,
      lastBlock: new Date()
    };
    return _this.ds.update('ion_sec_fail_login', {username: {$eq: username}}, data);
  }
    ...
    checkFailedLogins(username)
      .then(fail => {
        if (fail) {
          let update = {};
          if (fail.lastBlock) {
            let lastBlockTime = moment().subtract(tempBlockTimeOut, 'minutes');
            if (moment(lastBlockTime).isBefore(fail.lastBlock)) {
              if (fail.blocks >= tempBlockCount) {
                let blockTime = moment().subtract(blockTimeOut, 'minutes');
                if (moment(blockTime).isBefore(fail.lastAttempt)) {
                  block = true;
                } else {
