 ...
 * @param {Integer} [options.tempBlocksPeriod]
...
function Auth(options) {
  ...
  var tempBlocksPeriod = options.tempBlocksPeriod || 1; // minutes
  ...
  function upsertFail(username, fail) {
    ...
    if (!fail) {
      ...
      if (update.tmpBlocks >= tempBlockCount) {
        let blockingTime = moment().subtract(tempBlocksPeriod, 'minutes');
        if (moment(blockingTime).isBefore(fail.firstBlock)) {
  ...
  function isBlocked(fail) {
    ...
    if (fail && fail.lastBlock) {
      ...
      if (moment(blockTime).isBefore(fail.lastBlock)) {
        ...
        return _this.ds.delete('ion_sec_fail_login', {username: {$eq: fail.username}})
          .then(() => {
  ...
  function isTmpBlocked({fail, block}) {
    if (fail && !block && fail.lastTmpBlock) {
      let tempBlockTime = moment().subtract(tempBlockTimeOut, 'minutes');
      if (moment(tempBlockTime).isBefore(fail.lastTmpBlock)) {
        block = true;
      } else {
        return _this.ds.update(
          'ion_sec_fail_login',
          {username: {$eq: fail.username}},
          {attempts: null, lastAttempt: null}
        );
      }
    }
    return {fail, block};
  ...
  this.signIn = function (username, password, done) {
    ...
    checkFailedLogins(username)
      .then(fail => isBlocked(username, fail))
      .then(status => status.block ? status : isTmpBlocked(username, status))
      .then(status => {
        if (status.block) {
