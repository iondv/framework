...
function Auth(options) {
    ...
    if (uid) {
      ...
      hasher.hash((err, hash) => {
        ...
        ds.fetch('ion_pwd_log', {filter: {[F.EQUAL]: ['$user', uid]}})
          .then((pwds) => {
            for (let i = 0; (i < pwds.length) && (i < passwordJournalSize); i++) {
              if (pwds[i].hash == hash) {
                cb('Недопустимо использовать один из пяти предыдущих паролей.');
                return;
              }
            }
            cb(null, true);
  ...
  function changePwdHandler(self, module) {
    return function (req, res) {
      if (req.isAuthenticated()) {
        if (isBefore(passwordMinPeriod, req.user.pwdDate())) {
