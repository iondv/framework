...
function Auth(options) {
  ...
  function regAuthFail(username, fail) {
    ...
    if (!fail) {
      ...
      return ds.update(failsRegistry, {username: {$eq: username}}, update);
    }
  }
  ...
  function signInMin(username, password) {
    let checker = pwdHasher(password);
    return ds.get('ion_user',
      {
        $and: [
          {type: 'local', id: username},
          {$or: [
              {disabled: false},
  ...
  function signIn(req, username, password, done) {
    ds.get(failsRegistry, {username: username})
      ...
      .then(user => {
        return ds.delete(failsRegistry, {username: username})
    ...
    return function (req, res, next) {
      ...
      validate(req.body)
        .then(output => {
          ...
          return ds.update(
            'ion_user',
            {id: req.user[0].id, type: req.user[0].type},
            updates
          );
        })
        .then(user => {
  ...
  function verifier(redirect, module) {
    return function (req, res, next) {
      ...
      if (!req.isAuthenticated()) {
        if (!req.header('Referer') && isNotStatic(req.originalUrl)) {
      ...
      if (options.acl && options.checkUrlAccess) {
        let user = req.user[0];
        checkPathAccess(user.name + '@' + user.type, req.path)
          .then((can)=>{
            if (!can) {
        ...
        validatePassword(req.body, function (err) {
          ...
          hasher.verifyAgainst(req.user[0].pwd, function (err, verified) {
            ...
            hasher.hash(function (err, hash) {
              ...
              ds.update('ion_user', {id: req.user[0].id}, {pwd, pwdDate}).then(function (u) {
  ...
  this.bindAuth = function (a, module, auth, register, success, chpwd = 'chpwd') {
    ...
    if (options.denyTopLevel) {
      ...
      //a.get(prefix + module + '/profile', profileFormHandler());
      //a.get(prefix + module + '/' + chpwd, changePwdFormHandler());
      a.get(prefix + module + '/sign-out', signOut('/auth'));
  ...
  this.findUser = function (search) {
    return ds.fetch('ion_user', {filter: {id: {$regexp: '^' + search + '.*$', $options: 'i'}}, count: 10})
      .then((users) => {
        let result = {};
        users.forEach((u) => {result[u.id + '@' + u.type] = u.id;});
        return result;
      });
  };

  this.disableUser = function (id) {
    return ds.update('ion_user', {id: id}, {disabled: true}).then(()=>true);
  };

  this.enableUser = function (id) {
    return ds.update('ion_user', {id: id}, {disabled: false}).then(()=>true);
  };
