...
function Auth(options) {
        ...
        function (err, user) {
          if (err) {
            ...
            req.logIn(user, (err) => {
              ...
              return options.eventLogger.logChange(
                  eventLoggerTypes.LOGIN,
                  {
                    id: user.id(),
                    name: user.name(),
                    ip: user.properties().ip
                  }
                )
  ...
  function profileHandler(self, success, module) {
    return function (req, res, next) {
      ...
      validate(req.body)
        .then(output => options.eventLogger.logChange(
            eventLoggerTypes.EDIT_PROFILE,
            {
              id: req.user.id(),
              name: req.user.name(),
              ip: req.user.properties().ip,
              before: req.user.properties(),
              updates: output
            }
          )
