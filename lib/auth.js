...
function Auth(options) {
  ...
  var inactiveTimeout = options.inactiveTimeout || 15;
  var regFormFields = {};
  var regPreprocessor = Promise.resolve({});
  ...
  this.registerHandler = function (self, success) {
    return function (req, res, next) {
      ...
      for (let key of Object.keys(regFormFields)) {
        if (regFormFields[key] && regFormFields[key].required) {
          if (!req.body[key]) {
            req.flash('error', 'Не заполнены обязательные поля');
            return res.redirect(self);
          }
        }

        if (req.body[key] && ['username', 'password', 'password2', 'name', 'pwd'].indexOf(key) < 0) {
   ...
   * @param {Function} prep
   */
  this.injectPlugins = function (fields, prep) {
    if (fields) {
      regFormFields = fields;
    }
    if (prep) {
      regPreprocessor = prep;
    }
  };
}
