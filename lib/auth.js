...
function Auth(options) {
  ...
  function _block(req, res, redirect) {
    res.redirect(redirect || '/block');
  }
    ...
    return function (req, res, next) {
      passport.authenticate('local',
        function (err, user) {
          ...
          if (!user) {
            checkFailedLogins(req.body.username)
              .then(fails => {
                let promise;
                if (!fails) {
                  let data = {
                  ...
                  promise = _this.ds.insert('ion_sec_fail_login', data);
                } else {
                  let update = {
                    attempts: fails.attempts ? fails.attempts + 1 : 1,
                    lastAttempt: new Date()
                  };
                  promise = _this.ds.update('ion_sec_fail_login', {username: {$eq: req.body.username}}, update);
                }
                promise.then(() => {
  ...
  this.blockViewHandlrer = function (redirect) {
    return function (req, res, next) {
      if (req.isAuthenticated()) {
  ...
  this.bindAuth = function (a, module, auth, register, chpwd, block, success) {
    var prefix = module ? '/' : '';
    if (options.denyTopLevel) {
      ...
      a.get(prefix + module + '/' + block, this.blockViewHandlrer(prefix + module + '/'));
    } else {
  ...
  function checkBlockedMiddle(req, res, next) {


    checkFailedLogins(req.user[0])
  .then(failed => {
      if (failed) {
        let block = false;
        let update = {};
        /*
          if (failed.lastBlocked) {
            let blockTime = moment().subtract(blockTimeout, 'years');
            if (moment(blockTime).isBefore(failed.lastBlocked)) {
              block = true;
            } else {
              update.lastBlocked = null;
            }
          }
          if (!block && failed.lastTempBlocked) {
            let tempBlockTime = moment().subtract(tempBlockTimeout, 'minutes');
            if (moment(tempBlockTime).isBefore(failed.lastTempBlocked)) {
              block = true;
            } else {
              update.lastTempBlocked = null;
            }
          }*/
        console.log('failed', failed);
        if (!block && failed.attempts >= failLoginsCount) {
          console.log('attempts', failed.attempts, failLoginsCount);
          let tempBlockTime = moment().subtract(tempBlockTimeout, 'minutes');
          if (moment(tempBlockTime).isBefore(failed.lastAttempt)) {
            block = true;
          } else {
            update.lastAttempt = null;
          }
        }
        console.log('block', block, 'update', update);
        if (Object.keys(update).length) {
          _this.ds.update('ion_sec_fail_login', {username: {$eq: req.user[0].name}}, update).catch(console.log);
        }
        if (block) {
          return _block(req, res, `${module ? '/' + module : ''}/block`);
        }
      }
      if (!livePassword(req.user[0])) {
        return _changePwd(req, res, `${module ? '/' + module : ''}/chpwd`);
      } else if (!active(req.session.lastActive)) {
        return _exit(req, res, redirect || (module ? '/' + module : '') + '/auth');
      } else {
  ...
  function topLevelSetup() {
    if (!options.denyTopLevel && _this.app) {
      _this.app.use(checkBlockedMiddle);
      ...
      _this.app.get('/block', _this.blockViewHandlrer('/'));
    }
  }
