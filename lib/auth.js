 ...
 * @param {Integer} [options.failLoginsCount]
 * @param {Integer} [options.failLoginTimeOut]
 * @param {Integer} [options.tempBlockTimeOut]
 * @param {Integer} [options.tempBlockCount]
 * @param {Integer} [options.blockTimeOut]
 */
function Auth(options) {
  ...
  var failLoginsCount = options.failLoginsCount || 3;
  var failLoginTimeOut = options.failLoginTimeOut  || 1; // minutes
  var tempBlockTimeOut = options.tempBlockTimeOut || 1; // minutes
  var tempBlockCount = options.tempBlockCount || 3;
  var blockTimeOut = options.blockTimeOut || 100; // years
  ...
  function upsertFail(username, fail) {
    let now = new Date();
    if (!fail) {
      let data = {
        username,
      ...
      return _this.ds.insert('ion_sec_fail_login', data);
    } else {
      let update = {
        attempts: fail.attempts ? fail.attempts + 1 : 1,
        lastAttempt: now
      };
      if (update.attempts >= failLoginsCount) {
        update.tmpBlocks = fail.tmpBlocks ? fail.tmpBlocks + 1 : 1;
        update.lastTmpBlock = now;
        if (update.tmpBlocks === 1) {
          update.firstTmpBlock = now;
        }
      }
      if (update.tmpBlocks >= tempBlockCount) {
        update.blocks = fail.blocks ? fail.blocks + 1 : 1;
        update.lastBlock = now;
      }
      return _this.ds.update('ion_sec_fail_login', {username: {$eq: username}}, update);
    }
  }

  function isBlocked(username, fail) {
    let block = false;
    if (fail && fail.lastBlock) {
      let blockTime = moment().subtract(blockTimeOut, 'minutes');
      if (moment(blockTime).isBefore(fail.lastBlock)) {
        block = true;
      } else {
        return _this.ds.delete('ion_sec_fail_login', {username: {$eq: username}})
          .then(() => {
            return {block, fail: null};
          });
      }
    }
    return {block, fail};
    ...
    /*
      ...
      if (!block && fail.lastAttempt) {
        let failTime = moment().subtract(failLoginTimeOut, 'minutes');
        if (moment(failTime).isBefore(fail.lastAttempt)) {
          if (fail.attempts >= failLoginsCount) {
            let tempBlockTime = moment().subtract(tempBlockTimeOut, 'minutes');
            if (moment(tempBlockTime).isBefore(fail.lastAttempt)) {
              block = true;
            } else {
  ...
  function isTmpBlocked({fail, block}) {
    if (fail.tmpBlocks >= tempBlockCount) {
      let tempBlockTime = moment().subtract(tempBlockTimeOut, 'minutes');
      if (moment(tempBlockTime).isBefore(fail.firstTmpBlock)) {
        block = true;
      } else {
        update.tmpBlocks = null;
        update.lastTmpBlock = null;
      }
    }
    return {fail, block};
  }

  function isFailedAttempt(fail) {
    return fail;
  }

  this.signIn = function (username, password, done) {
    var checker = pwdHasher(password);
    checkFailedLogins(username)
      .then(fail => isBlocked(fail))
      .then(status => status.block ? status : isTmpBlocked(status))
      .then(status => status.block ? status : isFailedAttempt(status))
      .then(status => {
        if (status.block) {
          return done(null, false, {message: 'Вход заблокирован.'});
        } else {
          return _this.ds.fetch('ion_user', {filter: {type: 'local', id: username}})
            .then(users => {
              if (users.length > 0) {
                if (!users[0].pwd) {
                  return done(null, false, {message: 'Отсутствует пароль'});
                }
                checker.verifyAgainst(users[0].pwd, function (err, verified) {
                  if (verified) {
                    ...
                    return upsertFail(username, status.fail)
                      .then(() => done(null, false, {message: 'Неверно указан пароль.'}));
                  }
                });
              } else {
                return upsertFail(username, status.fail)
                  .then(() => done(null, false, {message: 'Пользователь не зарегистрирован.'}));
              }
            });
        }
      })
  ...
  function checkFailedLogins(username) {
    return _this.ds.fetch('ion_sec_fail_login', {filter: {username: {$eq: username}}})
      .then(fails => {
        if (!fails.length) {
          return null;
        }
    ...
    return function (req, res, next) {
      passport.authenticate('local',
        function (err, user, info) {
          ...
          if (!user) {
            req.flash('error', info && info.message ? info.message : 'Не удалось выполнить вход в систему.');
            return res.redirect(self);
          }
  ...
  this.changePwdFormHandlrer = function (module) {
    return function (req, res, next) {
  ...
  this.changePwdHandlrer = function (self, module) {
    return function (req, res, next) {
      if (req.isAuthenticated() && !livePassword(req.user[0])) {
        validatePassword(req.body, function (err) {
          if (err) {
  ...
  this.bindAuth = function (a, module, auth, register, chpwd, success) {
    ...
    if (options.denyTopLevel) {
      ...
      a.get(prefix + module + '/' + chpwd, this.changePwdFormHandlrer(module));
      a.post(prefix + module + '/' + chpwd, this.changePwdHandlrer(prefix + module + '/' + chpwd, module));
    } else {
