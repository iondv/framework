...
function Auth(options) {
  ...
  function upsertFail(username, fail){
    ...
    if (!fail) {
      ...
      promise = _this.ds.update('ion_sec_fail_login', {username: {$eq: username}}, update);
    }
    return promise;
  }
  ...
  this.signIn = function (username, password, done) {
    ...
    checkFailedLogins(username)
      .then(fail => {
        ...
        if (fail) {
          let update = {};
          if (!block && fail.attempts >= failLoginsCount) {
          ...
          if (Object.keys(update).length) {
            _this.ds.update('ion_sec_fail_login', {username: {$eq: username}}, update);
          }
        }
        if (block) {
