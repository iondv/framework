 ...
 * @param {DsChangeLogger2} options.eventLogger
 */
function Auth(options) {
    ...
    ds.get(failsRegistry, {[F.EQUAL]: ['$username', username]})
      ...
      .then(
        fail => signInMin(username, password)
          .then((user) => {
            if (!user) {
              return options.eventLogger.logChange(eventLoggerTypes.FAILURE, {name: username, ip: req.ip})
                .then(() => {
  ...
  function _exit(req, res, exit) {
    ...
    if (req.isAuthenticated() && req.user) {
      promise = options.eventLogger.logChange(eventLoggerTypes.LOGOUT,
  ...
  function validatePassword(body, cb, uid) {
    ...
    if (passwordComplexity && !validatePasswordComplexity(body.password, passwordComplexity)) {
      let msg = 'Пароль не прошел проверку на сложность. Пароль должен содержать ';
      let opts = [];
      if (passwordComplexity.upperLower) {
        opts.push('буквы в нижнем и верхнем регистре');
      }
      if (passwordComplexity.number) {
        opts.push('цифры');
      }
      if (passwordComplexity.special) {
        opts.push('специальные символы (@, $, & и т.д.)');
      }
      msg = msg + opts.join(opts.length < 3 ? 'и' : ',') + '.';
      return cb(msg, false);
    }
          ...
          pwds2.forEach((pwd) => {
            p = p.then(() => new Promise((resolve, reject) => {
              hasher.verifyAgainst(pwd.hash, (err, verified) => {
                ...
                if (verified) {
                  reject('Недопустимо использовать один из ' + passwordJournalSize + ' предыдущих паролей.');
                  return;
                }
                resolve();
              });
    ...
    return function (req, res, next) {
      ...
      validatePassword(req.body, (err) => {
        ...
        validate(req.body)
          ...
          .then(user => _this.register(user, function (err) {
            if (err) {
              log.error(err);
              req.flash('error', 'Не удалось зарегистрировать пользователя!');
              return res.redirect(self);
            }
            return options.eventLogger.logChange(eventLoggerTypes.REGISTER, {id: user.id(), name: user.name(), ip: user.properties().ip})
              .then(() => logPassword(user.id(), user.pwdHash()))
              .then(() => {
    ...
    return function (req, res, next) {
      ...
      validate(req.body)
        .then(output => options.eventLogger.logChange(eventLoggerTypes.EDIT_PROFILE,
            {id: req.user.id(), name: req.user.name(), ip: req.user.properties().ip, base: req.user.properties(), undates: output})
    ...
    return function (req, res) {
      if (req.isAuthenticated()) {
        if (!req.user.needPwdReset() && isBefore(passwordMinPeriod, req.user.pwdDate())) {
          req.flash('error', 'Не закончился период запрета на изменение пароля.');
          return res.redirect(self);
        }
        validatePassword(
          req.body,
      ...
      validatePassword(
        ...
        (err) => {
          if (err) {
            ...
            log.error(err);
            res.setStatus(500).send('Внутренняя ошибка сервера');
            return;
          }
  ...
  this.bindExternalAuth = function (a, nm, path, claim, success, fail) {
    ...
    function endMiddlware(req, res) {
      if (req.user) {
        ...
        return options.eventLogger.logChange(eventLoggerTypes.FAILURE, {name: ???, ip: req.ip})
          .then(() => {
