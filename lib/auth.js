...
function Auth(options) {
  ...
  function signInMin(username, password, type) {
    ...
    return ds.get('ion_user',
      ...
      .then((data) => {
        result.properties = Object.assign(result.properties || {}, data);
        return result;
      });
  }
  ...
  function _exit(req, res, exit) {
    if (req.isAuthenticated() && req.user) {
      ...
      logAuthEvent(req.user.name, 'sign out');
      req.logout();
    }
        ...
        function (err, user) {
          if (err) {
            ...
            req.logIn(user, (err) => {
              ...
              _this.emit('login', req.body.username, req.body.password, new User(user));
              ...
              let u = new User(user);
              return (options.acl ? options.acl.getCoactors(u.id()) : Promise.resolve([]))
                .then((coactors)=> {
      ...
      validate(req.body)
        .then(output => {
          ...
          return ds.update(
            ...
            {
              [F.AND]: [
                {[F.EQUAL]: ['$id', req.user.id]},
                {[F.EQUAL]: ['$type', req.user.type]}
              ]
            },
            updates
          );
    ...
    return function (req, res) {
      if (req.isAuthenticated()) {
        validatePassword(req.body, function (err) {
          ...
          hasher.verifyAgainst(req.user.pwd, function (err, verified) {
            if (err) {
            ...
            hasher.hash((err, hash) => {
              ...
              ds.update('ion_user', {[F.EQUAL]: ['$id', req.user.id]}, {pwd, pwdDate})
                .then(() => {
      ...
      if (user.id) {
        ds.fetch('ion_user', {
          ...
          if (users.length > 0) {
            ...
            _this.emit('login', users[0].id, users[0].password, new User(users[0]));
            return done(null, users[0]);
          }

          ds.insert('ion_user', user).then(function (user) {
            if (user) {
              _this.emit('login', user.id, user.password, newUser(user));
  ...
  this.getUser = function (req) {
    ...
    let u = req.user;
    if (u) {
      return new User(u, req.session.coactors, req.session.clientTimeZone);
    }
    return anon;
  };
