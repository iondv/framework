...
function Auth(options) {
  ...
  function livePassword({pwdTime}) {
    let liveTime = moment().substract(passwordLifetime, 'days');
    if (pwdTime) {
      return moment(liveTime).isBefore(pwdTime);
    }
    return false;
  }
  ...
  this.changePwdHandlrer = function (self, success) {
    return function (req, res, next) {
      if (req.isAuthenticated() && !livePassword(req.user[0])) {
        ...
        var hasher = pwdHasher(req.body.password);
        hasher.hash(function (err, hash) {
          let pwd = hash;
  ...
  this.bindAuth = function (a, module, auth, register, chpwd, success) {
    ...
    if (options.denyTopLevel) {
      ...
      a.post(prefix + module + '/' + chpwd, this.changePwdHandlrer(prefix + module + '/' + chpwd, success));
    } else {
