...
function Auth(options) {
  ...
  function signInMin(username, password, type) {
    ...
    return options.accounts.get(username, password)
      ...
      .then(data => result ? result.setProperties(data) : result);
  }
  ...
  function passportSetup() {
    ...
    if (typeof options.passports !== 'undefined') {
      for (let i = 0; i < options.passports.length; i++) {
        let pc = options.passports[i];
        if (pc && typeof pc === 'object' && pc.name && pc.strategy && pc.mapping) {
          ...
          let init = pc.options || {};
          init.auth = _this;
          passport.use(pc.name, new Strategy(init, authHandlerConstructor(pc.name, pc.mapping)));

          if (!options.denyTopLevel && app) {
            _this.bindExternalAuth(app, pc.name, '', pc.claim || {}, pc.success || null);
          }
        }
      }
    }
