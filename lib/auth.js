...
var AuthForm = module.exports.authFormHandler = function (onAuthenticated, module) {
  return function (req, res) {
...
var RegisterForm = module.exports.registerFormHandler = function (module) {
  return function (req, res) {
    if (req.isAuthenticated()) {
      delete req.session.authEntryPoint;
      req.logout();
...
var RegisterHandler = module.exports.registerHandler = function (self, success) {
  return function (req, res, next) {
    ...
    Register({name: req.body.username, pwd: req.body.password}, function (err) {
      if (err) {
        console.error(err); // TODO Реализовать логгирование ошибки
...
module.exports.bindAuth = function (a, module, auth, register, success) {
  var prefix = module ? '/' : '';
  a.get(prefix + module + '/' + auth, AuthForm(success, module));
  a.get(prefix + module + '/sign-out', SignOut(prefix + module + '/' + auth));
  a.get(prefix + module + '/' + register, RegisterForm(module));
  a.post(prefix + module + '/' + register, RegisterHandler(prefix + module + '/' + register, success));
  a.post(prefix + module + '/' + auth, SignInHandler(prefix + module + '/' + auth, success));
  a.use(prefix + module, Verifier(prefix + module + '/' + auth));
};
...
module.exports.init = function (app, conf) {
  ...
  return authDatasource.ensureIndex('ion_user', {type: 1, id: 1}, {unique: true});
};
