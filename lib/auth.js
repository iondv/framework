...
function Auth(options) {
      ...
      .catch(err => {
        if (typeof err.failInfo !== 'undefined') {
          regAuthFail(username, err.failInfo)
            .then(onAuthFail(username, err, done))
  ...
  function registerHandler(self, success, module) {
    return function (req, res, next) {
      ...
      validatePassword(req.body, function (err) {
        ...
        validate(req.body)
          .then(output => {return Object.assign(user, output);})
          .then(user => {
  ...
  function changePwdFormHandlrer(module) {
    return function (req, res, next) {
  ...
  this.bindAuth = function (a, module, auth, register, success, chpwd = 'chpwd') {
    ...
    if (options.denyTopLevel) {
      ...
      a.get(prefix + module + '/' + chpwd, changePwdFormHandlrer(module));
  ...
  function topLevelSetup() {
    if (!options.denyTopLevel && app) {
      ...
      app.get('/chpwd', changePwdFormHandlrer());
