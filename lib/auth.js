 ...
 * @param {Boolean} [options.noPublicRegistration]
...
function Auth(options) {
  ...
  var renderAuthForm = function (req, res, module) {
    res.status(401).render('login', {
      module: module,
      canRegister: !options.noPublicRegistration,
      errors: req.flash('error')
    });
  };

  var renderRegisterForm = function (req, res, module) {
    if (options.noPublicRegistration) {
      res.sendStatus(404);
    } else {
    ...
    return function (req, res, next) {
      if (Array.isArray(options.exclude)) {
        ...
        for (var i = 0; i < options.exclude.length; i++) {
          if (options.exclude[i]) {
            try {
              reg = new RegExp(options.exclude[i]);
      ...
      if (!req.isAuthenticated()) {
        ...
        return res.redirect(redirect || '/auth');
      } else {
        _this.emit('verify', req.user[0], function () {
          _exit(req, res, redirect || '/auth');
        });
      }
      next();
    };
