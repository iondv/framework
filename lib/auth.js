...
function Auth(options) {
  ...
  var passwordMinPeriod = parseDuration(options.passwordMinPeriod || '0h');
  ...
  var passwordComplexity = options.passwordComplexity ?
    options.passwordComplexity :
  {upperLower: true, number: true, special: true};
  var passwordJournalSize = options.passwordJournalSize || 5;
  var changePwdUrl = options.changePwdUrl;
  ...
  if (!ds) {
    throw new Error('No authentication data source is configured.');
  }
  ...
  /**
   * Method of obtaining the current user account
   * @param {String} id
  ...
  function aclChecker(user, module) {
    if (options.acl && options.checkSignInAccess) {
      return options.acl
        ...
        .then((can) => {
          if (!can) {
            let err = new Error('Not enough permissions to login.');
            err.failInfo = true;
            throw err;
          }
          return user;
  ...
  function throwBlocked(fail, req) {
    ...
    let err = new Error('Account locked till ' + d.format('L LT'));
    err.failInfo = fail;
    return err;
  }
  ...
  function renderAuthForm(req, res, module) {
    res.status(401).render('login', {
      ...
      errors: req.session && req.flash('error') || ['For technical reasons, login is temporarily unavailable.']
    });
  }
    ...
    return function (req, res, next) {
      ...
      if (!req.body.password) {
        if (passwordMinLength) {
          req.flash('error', 'User password not specified!');
          return res.redirect(self);
        } else {
          req.body.password = '';
        }
  ...
  function validatePassword(body, cb, uid) {
    if (!body.password) {
      return cb('Password not passed!', false);
    }

    if (body.password !== body.password2) {
      return cb('Wrong password repeat!', false);
    }

    if (body.password.length < passwordMinLength) {
      return cb('Minimum password length: ' + passwordMinLength + ' symbols', false);
    }

    if (passwordComplexity && !validatePasswordComplexity(body.password, passwordComplexity)) {
      let msg = 'Password didn\'t pass the complexity check. Password must contain ';
      let opts = [];
      if (passwordComplexity.upperLower) {
        opts.push('lowercase and uppercase letters');
      }
      if (passwordComplexity.number) {
        opts.push('digits');
      }
      if (passwordComplexity.special) {
        opts.push('special characters (@, $, & and etc.)');
      }
      msg = msg + opts.join(opts.length < 3 ? 'and' : ',') + '.';
      return cb(msg, false);
    }
          ...
          pwds2.forEach((pwd) => {
            p = p.then(() => new Promise((resolve, reject) => {
              hasher.verifyAgainst(pwd.hash, (err, verified) => {
                ...
                if (verified) {
                  reject('You can\'t use one of the previous ' + passwordJournalSize + ' passwords.');
                  return;
                }
                resolve();
              });
  ...
  function registerHandler(self, success, module) {
    return function (req, res, next) {
      if (!options.publicRegistration) {
        req.flash('error', 'Public registration of users is prohibited!');
        return res.redirect(self);
      }
        ...
        validate(req.body)
          ...
          .then((user) => {
            _this.register(user, function (err) {
              ...
              logPassword(user.id(), user.pwdHash())
                .then(() => {
  ...
  function profileHandler(self, success, module) {
    return function (req, res, next) {
      ...
      validate(req.body)
        ...
        .catch((err) => {
          log.error(err);
          req.flash('error', 'Failed to make changes to user profile!');
          return res.redirect(self || '/profile');
        });
    };
  }
    ...
    return function (req, res) {
      if (req.isAuthenticated()) {
        if (!req.user.needPwdReset() && isBefore(passwordMinPeriod, req.user.pwdDate())) {
          req.flash('error', 'Not over the period of the ban to change the password.');
          return res.redirect(self);
        }
        validatePassword(
          req.body,
  ...
  this.bindAuth = function (a, module, routes) {
    ...
    if (options.denyTopLevel) {
      ...
      if (chpwd) {
        ...
        a.use(app.locals.baseUrl, (req, res, next) => {
  ...
  this.bindExternalAuth = function (a, nm, path, claim, success, fail) {
    ...
    function endMiddlware(req, res) {
      if (req.user) {
        ...
        req.flash('error', 'User was not authenicated.');
        ...
        options.accounts.get(user.id + '@' + user.type)
          ...
          .then(
            (u) => {
              ...
              throw new Error('Failed to register user from external system.');
            })
          .catch(err => done(err));
      } else {
        return new Error(done('Could not determine user ID.'));
      }
    };
  }
  ...
  function topLevelSetup() {
    ...
    if (!options.denyTopLevel && app) {
      ...
      app.post(app.locals.baseUrl + 'checkPwd', checkPwdHandler());
      ...
      app.use(app.locals.baseUrl, (req, res, next) => {
        if (req.user && req.user.needPwdReset()) {
          res.redirect(app.locals.baseUrl + (changePwdUrl || 'chpwd'));
          return;
        }
        ...
        if (passports.hasOwnProperty(nm)) {
          ...
          if (pc.strategy) {
            ...
            if (pc.strategy instanceof StrategyProvider) {
              ...
              throw new Error('No strategy specified for passport ' + nm);
            }
