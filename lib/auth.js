 ...
 * @param {Integer} [options.fullBlockTimeOut]
 */
function Auth(options) { // jshint ignore:line
  var _this = this;
  ...
  var attemptTimeOut = parseDuration(options.attemptTimeOut || '5m');
  var attemptLimit = options.attemptLimit || 10;
  var tempBlockTimeOut = parseDuration(options.tempBlockTimeOut || '5m');
  var tempBlockLimit = options.tempBlockLimit || 3;
  var tempBlocksPeriod = parseDuration(options.tempBlocksPeriod || '10m');
  var fullBlockTimeOut = parseDuration(options.fullBlockTimeOut || '120d');
  ...
  function upsertFail(username, fail) { // jshint ignore:line
    let now = new Date();
    if (!fail) {
      let data = {
        username,
      ...
      if (update.attempts >= attemptLimit) {
  ...
  function aclChecker(username) {
    if (options.acl) {
      return options.acl.checkAccess(username, '__auth', Permissions.USE);
    }
    return Promise.resolve(false);
  }

  function isBlocked(username) {
    return aclChecker(username)
      .then(blocked => {
        if (blocked) {
          return {blocked};
        }
        return checkFailedLogins(username)
          .then(fail => {
            let blocked = false;
            if (fail && fail.blockedTill) {
              if (moment().isBefore(fail.blockedTill)) {
                ...
                return _this.ds.update('ion_sec_fail_login', {username: {$eq: fail.username}}, {blockedTill: null})
                 .then(fail => ({blocked, fail}));
              }
            }
            return {blocked, fail};
          });
  ...
  function authLog(username, type) {
    let data = {
      username,
      type,
      date: new Date()
    };
    _this.ds.insert('ion_sec_log', data).catch(console.log);
  }
  ...
  this.signIn = function (username, password, done) {
    ...
    isBlocked(username)
      .then(({blocked, fail}) => {
        if (blocked) {
          return {user: false, info: {message: 'Вход заблокирован.', blocked: true}};
        } else {
          return _this.ds.fetch('ion_user', {filter: {type: 'local', id: username}})
            .then(users => {
              if (users.length > 0) {
                if (!users[0].pwd) {
                  return {user: false, info: {message: 'Отсутствует пароль'}};
                }
                return new Promise(function (resolve, reject) {
                  checker.verifyAgainst(users[0].pwd, function (err, verified) {
                    if (verified) {
                      return resolve({err: null, user: users[0], info: null});
                    } else {
                      if (err) {
                        return resolve({err, user: false});
                      }
                      return upsertFail(username, fail)
                        .then(() => resolve({user: false, info: {message: 'Неверно указан пароль.'}}));
                    }
                  });
                });
              } else {
                return upsertFail(username, fail)
                  .then(() => ({user: false, info: {message: 'Пользователь не зарегистрирован.'}}));
              }
            });
        }
      })
      .then(({err, user, info}) => {
        ...
        let type = user ? 'login' : 'failedLogin';
        authLog(username, type);
      })
  ...
  function _exit(req, res, exit) {
    if (req.user[0]) {
      authLog(req.user[0].name, 'logout');
    }
    ...
    return function (req, res, next) {
      passport.authenticate('local',
        function (err, user, info) {
          ...
          if (!user) {
            req.flash('error', info && info.blocked ? 'Вход заблокирован.' : 'Не удалось выполнить вход в систему.');
            return res.redirect(self);
          }
  ...
  this.bindAuth = function (a, module, auth, register, success, chpwd='chpwd') {
    var prefix = module ? '/' : '';
