...
function Auth(options) {
  ...
  this.bindAuth = function (a, module, routes) {
    ...
    if (options.denyTopLevel) {
      ...
      if (auth) {
        ...
        a.use(prefix + module, verifier(prefix, module));
      }
      if (profile) {
  ...
  function topLevelSetup() {
    ...
    if (!options.denyTopLevel && app) {
      ...
      app.post(app.locals.baseUrl + 'register', registerHandler(app.locals.baseUrl + 'register'));

      app.post(app.locals.baseUrl + 'auth', signInHandler(app.locals.baseUrl + 'auth'));
