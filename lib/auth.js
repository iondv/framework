...
// jshint maxstatements: 60, maxcomplexity: 30
/**
 *
...
function Auth(options) {
  ...
  function checkPathAccess(user, path) {
    return options.acl.checkAccess(user, urlResource + ':' + path, Permissions.USE)
      .then((can)=>can || options.acl.checkAccess(user, urlResource + ':' + path + '/', Permissions.USE))
      .then((can)=>can || options.acl.checkAccess(user, urlResource + ':' + path + '/*', Permissions.USE))
      .then((can)=>{
