        ...
        function (err, user) {
          if (err) {
            ...
            req.logIn(user, function (err) {
              ...
              console.log(req.session.authEntryPoint, req.cookies, success);
              var redir = req.session.authEntryPoint && req.session.authEntryPoint !== '/' ?
                req.session.authEntryPoint :
                req.cookies && req.cookies.lastUrl ?
                  req.cookies.lastUrl :
                  success || '/';
              delete req.session.authEntryPoint;
              if (options.acl) {
