  ...
  function passportSetup() {
    ...
    if (typeof options.passports !== 'undefined') {
      for (let i = 0; i < options.passports.length; i++) {
        ...
        if (pc && typeof pc === 'object' && pc.name && pc.module && pc.mapping) {
          let Strategy = require(pc.module).Strategy;
          let init = pc.options || {};
          init.auth = _this;
          passport.use(pc.name, new Strategy(init, authHandlerConstructor(nm, pc.mapping)));
