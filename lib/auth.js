...
function Auth(options) {
    ...
    options.accounts.profileFields()
      .then((fields) => {
        res.status(200).render('profile', {
          user,
          __: (id, params) => req.app.locals.__(id, params, user.language()),
          languages: strings.getLangs(),
          fields,
          baseUrl: req.app.locals.baseUrl,
          module: module,
          options: {
  ...
  function profileHandler(self, success, module) {
    return function (req, res, next) {
      ...
      validate(req.body)
        .then(({language, properties}) => {
          const updates = {properties};
          if (language) {
            updates.language = language;
          }
          return options.accounts.set(req.user.id(), updates);
        })
        .then((user) => {
  ...
  this.bindExternalAuth = function (a, nm, path, claim, success, fail) {
    ...
    function endMiddlware(req, res) {
      if (req.user) {
        ...
        req.flash('error', __(Errors.NOT_AUTHENTICATED));
