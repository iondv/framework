...
function exportUsers(auth, am) {
  return auth.getUsers().then(users => {
    let promises = [];
    if (Array.isArray(users) && users.length) {
      users.forEach(u => {
        if (u.id) {
          promises.push(am.getRoles(`${u.id}@${u.type}`).then(rs => {
            return {
              id: u.id,
              name: u.name,
              pwd: {hash: u.pwd},
      ...
      return dataPromises
        .then(() => {
          ...
          if (options.exportAcl) {
            return exportAcl(dst, options.auth, options.accessManager, options.namespace, options.exportAcl);
          }
        });
    });
};
