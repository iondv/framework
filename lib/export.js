...
function exportData(dst, dataRepo, cm, offset) {
  return function () {
    return new Promise(function (resolve, reject) {
      ...
      mkdirp.sync(dst);
      dataRepo.getList(cm.getCanonicalName(),
        {
          offset: ofst,
          count: 100,
          nestingDepth: 0
        ...
        for (var i = 0; i < list.length; i++) {
          fn = path.join(dst, encodeURIComponent(list[i].getClassName() + '@' + list[i].getItemId() + '.json'));
          delete list[i].base._id;
          fs.writeFileSync(fn, JSON.stringify(list[i].base, null, 2), {encoding: 'utf-8'});
