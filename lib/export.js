...
function exportFile(f, dest) {
  return f.getContents()
    .then(
      function (f) {
        return new Promise(function (resolve, reject) {
          formPath(dest, f.name, 0, function (err, pth) {
            f.stream.pipe(fs.createWriteStream(pth, f.options))
              .on(
                'end',
                function () {
                  resolve({
 ...
 * @returns {Function}
 */
function writeObject(dst, item, dest) {
  return function () {
    ...
    for (var pn in props) {
      if (props.hasOwnProperty(pn)) {
        ...
        if (
          ...
          if (f) {
            if (dest) {
              fileLoaders.push(exportFile(f, dest));
            } else {
        ...
        if (prop.getType() === PropertyTypes.FILE_LIST) {
          ...
          if (files) {
            ...
            for (let i = 0; i < files.length; i++) {
              if (dest) {
                fileLoaders.push(exportData(files[i], dest));
              } else {
                fileLoaders.push(readFile(files[i]));
              }
            }
          }
        }
