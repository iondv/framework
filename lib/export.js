...
module.exports = function (dst, options) {
  ...
  function exportClass(dst, cm, paths, metaRepo, noVersion) {
    log('Export class' + cm.getCanonicalName());
  ...
  return options.metaRepo.init()
    .then(() => {
      ...
      // Export of meta and representation models and formation of data export promises
      ...
      metas.forEach((cm) => {
        ...
        if (!options.skipData && !cm.getAncestor()) {
          p = p.then(() => {
            log('Export class data ' + cm.getCanonicalName());
