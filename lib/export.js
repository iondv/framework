...
module.exports = function (dst, options) {
   ...
   * @returns {Promise}
   */
  function exportData(dst, cm, fileDir, offset) {
    ...
    return options.dataRepo.getList(
      ...
      (list) => {
        ...
        list.forEach((item) => {
          w = w.then(() => options.workflows.getStatus(item, {})).then(writeObject(dst, item, fileDir));
        });
        if (list.length === 100) {
          return w.then(() => exportData(dst, cm, fileDir, ofst + 100));
        } else {
          return w;
        }
      }
  ...
  function exportNode(dst, node, paths) {
    ...
    let p = new Promise((resolve, reject) => {
      fs.writeFile(fn, JSON.stringify(n, null, 2), {encoding: 'utf-8'}, (err) => {
    ...
    let p = new Promise((resolve, reject) => {
      mkdirp(dst, (err) => {
        ...
        fs.writeFile(fn, JSON.stringify(sect, null, 2), {encoding: 'utf-8'}, (err) => {
          ...
          mkdirp(dirName, (err) => {
            return err ? reject(err) : resolve(dirName);
          });
        });
      });
    });
  ...
  function exportClass(dst, cm, paths, metaRepo, noVersion) {
    ...
    let p = new Promise((resolve, reject) => {
      mkdirp(path.join(dst, 'meta'), (err) => {
        ...
        fs.writeFile(fn, JSON.stringify(c, null, 2), {encoding: 'utf-8'}, (err) => {
          return err ? reject(err) : resolve();
        });
      });
    });
  ...
  function exportResources(am, data) {
    return am.getResources()
      .then((resources) => {
        if (!Array.isArray(data.resources)) {
          data.resources = [];
        }
        resources.forEach((rs) => data.resources.push({id: rs.id, name: rs.name}));
      });
  }

  function exportRoles(am, data) {
    return am.getRoles()
      .then((roles) => {
        let p = Promise.resolve();
  ...
  function exportUsers(accounts, am, data) {
  ...
  function exportAcl(dst, accounts, am, namespace, type) {
    ...
    return exportResources(am, data)
      .then(() => exportRoles(am, data))
      .then(() => exportUsers(accounts, am, data))
      .then(() => {
        return new Promise((resolve, reject) => {
          mkdirp(path.join(dst, 'acl'), (err) => {
            if (err) {
              return reject(err);
            }
            let fn = path.join(dst, 'acl', `${name}.${type}`);
            fs.writeFile(fn, convert(data, type), {encoding: 'utf-8'}, (err) => {
              return err ? reject(err) : resolve();
            });
          });
        });
      ...
      p = p.then(() => {
        return new Promise((resolve, reject) => {
          mkdirp(path.join(dst, 'workflows'), (err) => {
            ...
            mkdirp(options.fileDir ? path.join(dst, 'data', options.fileDir) : path.join(dst, 'data'), (err) => {
              return err ? reject(err) : resolve();
            });
          });
        });
      ...
      metas.forEach((cm) => {
        ...
        if (!options.skipData && !cm.getAncestor()) {
          p = p.then(() => {
            ...
            return exportData(path.join(dst, 'data'), cm, options.fileDir);
          });
        }
      });
        ...
        p = p
          ...
          .then((snapshot) => {
            return new Promise((resolve, reject) => {
              ...
              fs.writeFile(fn, JSON.stringify(snapshot), {encoding: 'utf-8'}, (err) => {
                return err ? reject(err) : resolve();
              });
            });
          });
