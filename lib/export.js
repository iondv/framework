...
module.exports = function (dst, options) {
  ...
  function exportAcl(dst, accounts, am, namespace, type) {
    ...
    return mkdirPromise(path.join(destination, 'resources'))
      .then(resDir => exportResources(am, resDir, type2))
      .then(() => mkdirPromise(path.join(destination, 'roles')))
      .then(roleDir => exportRoles(am, roleDir, type2))
      .then(() => mkdirPromise(path.join(destination, 'users')))
      .then(userDir => exportUsers(accounts, am, userDir, type2));
  }
