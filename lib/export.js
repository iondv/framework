...
function exportData(dst, dataRepo, workflows, cm, fileDir, offset) {
  return function () {
    ...
    console.log(cm.getCanonicalName(), 'export data from', ofst);
    ...
    .then(() => {
      ...
      metas.forEach((cm) => {
        ...
        if (!options.skipData && !cm.getAncestor()) {
          p = p.then(exportData(path.join(dst, 'data'), options.dataRepo, options.workflows, cm, options.fileDir));
        }
      });
