...
module.exports.start = function (sourcePath, regionFilter, dataRepo, logger) {
  ...
  then(function () {
    var importer = getRecordImporter(dataRepo, isFias, logger, regionFilter);
    if (isFias === true) {
...
function getRecordImporter(dataRepo, isFias, regionFilter, logger) {
        ...
        if (item) {
          return dataRepo.bulkUpdate(
            ...
            return dataRepo.deleteItem(item.getClassName(), item.getItemId()).then(function () {
              logger.warn(`Удален из реестра более неактуальный адресный объект ${item.base.KLADR_CODE}.`);
              return Promise.resolve();
            });
          });
        }
