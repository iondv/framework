...
function IonDataRepository(options) {
    ...
    try {
      ...
      if (conditions) {
        ...
        return p.then(function () {
          ...
          if (!data) {
            return Promise.reject(new IonError(IonError.ERR_DR_REQUEST, null, `Не найден объект для редактирования ${cm.getName()}@${id}`));
          }
          var item = _this._wrap(data._class, data, data._classVer);
      ...
      return Promise.reject(errorHandle(err, cm, data));
    }
  };
    ...
    try {
      ...
      return p.then(function () {
          ...
          try {
            ...
            if (chr !== true) {
              return Promise.reject(errorHandle(chr, cm));
            }
  ...
  this._deleteItem = function (classname, id, changeLogger, options) {
    ...
    return p.
      ...
      }).catch(e => Promise.reject(errorHandle(e, cm)));
  };
